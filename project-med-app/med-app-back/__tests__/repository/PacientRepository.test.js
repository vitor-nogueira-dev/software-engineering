...
describe('Pacient Repository Test', () => {
  ...
  it('Should receive error when missing birth date', async () => {
    ...
    expect(async () => {
      ...
      new Error('ValidationError: birthDate: Birth Date is required.')
    );
  });
  ...
  it('Should receive error when missing birth date', async () => {
    ...
    expect(async () => {
      ...
      new Error('ValidationError: email: Email contact is required.')
    );
  });
  ...
  it('Should validate phone format', async () => {
    ...
    expect(async () => {
      ...
      new Error(
        'ValidationError: phone: 11988772201 This is not a valid phone value. Please use the following format 99 91234-4567'
      )
    );
  });
});
